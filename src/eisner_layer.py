...
def batch_parse(batch_scores):
    ...
    # CYK table
    complete_table = np.zeros((batch_size, sentence_length * sentence_length * 2), dtype=np.float)
    incomplete_table = np.zeros((batch_size, sentence_length * sentence_length * 2), dtype=np.float)
    # backtrack table
    complete_backtrack = -np.ones((batch_size, sentence_length * sentence_length * 2), dtype=np.int)
    incomplete_backtrack = -np.ones((batch_size, sentence_length * sentence_length * 2), dtype=np.int)
    ...
    for ij in ijss:
        ...
        incomplete_table[:, ij] = np.amax(span_i, axis=1)
        ...
        if dir == 0:
            ...
            span_c = ik_ic + kj_cc
        complete_table[:, ij] = np.amax(span_c, axis=1)
        max_idx = np.argmax(span_c, axis=1)
        complete_backtrack[:, ij] = max_idx

    heads = -np.ones((batch_size, sentence_length),dtype=np.long)
